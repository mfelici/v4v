...
,
code(ord,ordinal_position,code) AS (
SELECT
  1
, ordinal_position
, CASE row_number() OVER (PARTITION BY table_schema,table_name ORDER BY ordinal_position)
    WHEN 1 THEN 'INSERT INTO '||table_schema||'.'||table_name||'('||CHR(10)||'  '
    ELSE        ', '
  END
||column_name||REPEAT(' ',maxln - CHAR_LENGTH(column_name))
||CASE WHEN 
...
, CASE row_number() OVER (PARTITION BY table_schema,table_name ORDER BY ordinal_position)
    ...
    ELSE        ', '
  END
||column_name||REPEAT(' ',maxln - CHAR_LENGTH(column_name))||' -- '||column_name
||CASE WHEN 
    LEAD(column_name) OVER (PARTITION BY table_schema,table_name ORDER BY ordinal_position)
    IS NOT NULL 
    THEN ''
...
SELECT code FROM code ORDER BY ord,ordinal_position
;
